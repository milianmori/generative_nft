...
import { start, MidiClass, Oscillator, Gain, Transport, AmplitudeEnvelope, Synth} from 'tone';
...
startButton.addEventListener('click', async () => {
    ...
    gains = gains.map((item,index) => {
        return (new Gain({gain: exponentialGains(index)}).toDestination()); //connect(env)
    });
